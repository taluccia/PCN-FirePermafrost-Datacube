...
```{r}
f <- list.files(path = "../data/GoogleDriveCsv20240229/",
...
clean <- function(df){ df %>%
  ...
  rename(plotId = plot_id,
    ...
    dplyr::select(plotId, siteId, cntryId, lastNm, submitNm, lat, lon, year, month, day, biome, distur, fireYr, paired, gtProbe, hitRock, orgDpth, srfH2O, topoPos, slope, vegCvr, msrDoy, msrType, msrDepth) %>%
...
# Combine into datacube

```{r}
dataCube = dplyr::bind_rows(f1_clean, f2_clean, f3_clean, f4_clean, f5_clean, f6_clean, f7_clean, f8_clean, f9_clean, f10_clean, f11_clean, f12_clean, f13_clean, f14_clean, f15_clean, f16_clean, f17_clean, f18_clean)
```
...
```{r}
( nadoy = dataCube %>% filter(is.na(msrDoy)) )
```

```{r}
unique(nadoy$submitNm)

dataCube %>% filter(submitNm =="Dieleman")
```

## Check and fix factors

```{r}
unique(dataCube$hitRock)
unique(dataCube$gtProbe)
```

```{r}
dataCube$hitRock[dataCube$hitRock==""]<-"n"
dataCube$gtProbe[dataCube$gtProbe==""]<-"n"
dataCube$gtProbe[is.na(dataCube$gtProbe)]<-"n"
```
...
write_csv(missingDataSummary, '../outputs/dataCube/PermafrostFireContributedDataMissingDataSummary.csv')
```
...
```{r eval=FALSE, include=FALSE}
write_csv(dataCube, '../outputs/dataCube/PermafrostFireContributedData.csv')
```
...
```{r}
dataCube %>% filter(if_any(paired, is.na)) #166
dataCube %>% filter(hitRock == "y" ) #853
dataCube %>% filter(gtProbe == "y" ) #2237
dataCube %>% filter(if_any(msrDepth, is.na)) #440
dataCube %>% filter(day==-9999) #903
```

```{r}
( dataCubeFinal = dataCube %>% 
    drop_na(paired) %>% 
    filter(!is.na(msrDoy)) %>% 
...
```{r}
( difference = setdiff(dataCube,dataCubeFinal) )
...
```{r}
(pair = dataCube %>% filter(is.na(paired)) %>% dplyr::select(plotId, siteId, submitNm, paired) %>% group_by(plotId, siteId, submitNm) %>% summarize(n_paired = n()))
msrDoy = dataCube %>% filter(is.na(msrDoy))  
msrDepth = dataCube %>% filter(is.na(msrDepth))
hitrock = dataCube %>% filter(hitRock == "y" )  
gtprobe = dataCube %>%filter(gtProbe == "y" ) 
```
## Save Datacube as csv

```{r eval=FALSE, include=FALSE}
write_csv(dataCubeFinal, '../outputs/dataCube/PermafrostFireDataCube.csv')
```
...
## write to shapefile
```{r eval=FALSE, include=FALSE}
st_write(dataCubePts, "../outputs/dataCube/PermafrostFireDataCube.shp", driver="ESRI Shapefile")
```
