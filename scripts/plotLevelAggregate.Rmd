...
sitePlot1 %>% 
  dplyr::select(-lastNm) %>% 
  filter_all(any_vars(grepl(',', .))) # checks for data aggregates that have multiples that need to be combined
```
```{r}
firealt_tsf %>%  filter(plotId=="CG1-8B")
```
...
```{r}
write.csv(PlotLevel4, file = "../outputs/dataset/FireAltEstimatedPlotLevel.csv", row.names=FALSE)
```
